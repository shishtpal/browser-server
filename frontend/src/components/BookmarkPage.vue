  ...
  <div class="mx-auto max-w-full px-4 py-4 sm:px-6 lg:px-10 xl:px-12">
    <PageHeader badge="Saved links" title="Bookmarks" color="cyan">
      ...
      <template #controls>
        ...
        <div v-if="allTags.length" class="flex flex-wrap gap-1">
          <FilterPill :active="activeTagFilter === null" @click="activeTagFilter = null"
            >All</FilterPill
          >
          <FilterPill
    ...
    <div v-else-if="selectedUserId">
      ...
      <div
        ...
        class="mb-4 flex items-center gap-2 rounded-xl border border-cyan-200 bg-cyan-50/80 p-2 text-xs text-cyan-800 shadow-sm transition-colors dark:border-cyan-900/30 dark:bg-cyan-900/20 dark:text-cyan-300"
      >
        ...
        <button
          ...
          class="rounded-md bg-cyan-200 px-2 py-0.5 font-black text-cyan-800 transition hover:bg-cyan-300 dark:bg-cyan-800 dark:text-cyan-200 dark:hover:bg-cyan-700"
        >
          Clear
        </button>
      </div>
    ...
    <div
      v-else
      class="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-6 text-center shadow-sm transition-colors dark:border-slate-600 dark:bg-slate-800/60"
    >
      <div
        class="mx-auto grid h-10 w-10 place-items-center rounded-xl bg-gray-100 text-slate-400 transition-colors dark:bg-slate-700 dark:text-slate-500"
      >
      ...
      <h2 class="mt-3 text-base font-black text-slate-800 transition-colors dark:text-slate-200">
        Choose a workspace
      </h2>
      <p class="mt-1 text-xs text-slate-500 transition-colors dark:text-slate-400">
        Select a user to manage their bookmarks.
      </p>
    </div>

    <BookmarkEditModal :bookmark="editing" @close="editing = null" @save="handleSaveEdit" />
  </div>
</template>
