<template>
  <div class="mx-auto max-w-full px-4 py-4 sm:px-6 lg:px-10 xl:px-12">
    ...
    <EmptyState v-else-if="status && !enabled" title="Background tasks are disabled"
    ...
    <template v-else>
      ...
      <form
        ...
        <label for="task-prompt" class="mb-2 block text-xs font-black uppercase tracking-wider text-slate-500">
          New task
        </label>
        <textarea id="task-prompt" v-model="draft" rows="3" :disabled="isSubmitting"
          placeholder="Describe what the agent should do. It runs unattended, so avoid anything that needs your input."
          class="w-full resize-y rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-violet-400 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
          @keydown.ctrl.enter.prevent="handleSubmit" @keydown.meta.enter.prevent="handleSubmit" />
        <div class="mt-2 flex items-center justify-between gap-3">
          <p class="text-[11px] text-slate-500 dark:text-slate-400">
            {{ workers }} worker{{ workers === 1 ? '' : 's' }} · Ctrl+Enter to submit
          </p>
          <Button type="submit" variant="gradient-violet" size="sm" :loading="isSubmitting" loading-text="Queueing..."
            :disabled="!draft.trim()">
            Queue task
          </Button>
        </div>
      ...
      <div class="mb-4 flex flex-wrap items-center gap-2">
        <button v-for="option in filterOptions" :key="option.value" type="button"
          class="rounded-lg px-3 py-1.5 text-xs font-semibold transition" :class="filter === option.value
          ...
          {{ option.label }}
          <span v-if="countFor(option.value) !== null" class="ml-1 tabular-nums opacity-60">{{ countFor(option.value) }}</span>
        </button>
        <span v-if="hasActive" class="ml-auto flex items-center gap-1.5 text-[11px] font-medium text-violet-600 dark:text-violet-400">
          <span class="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-violet-500"></span>
          Live
        </span>
      </div>

      <EmptyState v-if="!tasks.length" title="No tasks yet"
        description="Queued work appears here and keeps running in the background, surviving restarts." icon="clock"
        color="violet" />

      <ul v-else class="space-y-3">
        <TaskCard v-for="task in tasks" :key="task.id" :task="task" @cancel="handleCancel" @delete="handleDelete" />
      </ul>
    </template>
  </div>
</template>
