<template>
  <div class="relative" ref="dropdownRef">
    <button
      type="button"
      @click="toggleOpen"
      class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 bg-white/70 text-slate-600 shadow-sm transition hover:bg-white hover:text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white"
    ...
    >
      <div
        v-if="open"
        class="absolute top-full right-0 z-50 mt-2 w-44 overflow-hidden rounded-2xl border border-gray-200 bg-white p-1.5 shadow-xl dark:border-slate-700 dark:bg-slate-900"
        role="menu"
      >
        <button
...
const options: { label: string; value: 'light' | 'dark' | 'system'; icon: typeof SunIcon }[] = [
