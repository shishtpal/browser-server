  ...
  >
    <div v-if="bookmark" :key="bookmark.id" class="grid gap-3">
      <input
        ...
        placeholder="Title"
        required
        class="rounded-lg border border-gray-300 bg-gray-50 px-3 py-2 text-sm font-semibold text-slate-700 focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100 focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:focus:ring-cyan-900/30"
      />
      <input
    ...
    <div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
      ...
      <button
        type="button"
        @click="onSave"
        class="rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-2 text-sm font-black text-white shadow-lg shadow-cyan-500/20"
      >
        Save changes
      </button>
    </div>
