<template>
  <div>
      ...
      >
        ...
        <thead class="bg-gray-50 transition-colors dark:bg-slate-800/80">
          <tr>
            ...
            <th
              class="w-28 px-3 py-3 text-right text-[10px] font-black tracking-wide text-slate-500 uppercase transition-colors dark:text-slate-400"
            >
              Actions
            </th>
          </tr>
        ...
        <tbody class="divide-y divide-gray-100 transition-colors dark:divide-slate-700/50">
          <tr
            ...
            :key="b.id"
            class="transition hover:bg-cyan-50/60 dark:hover:bg-cyan-900/20"
          >
            <td class="px-3 py-3">
              <div
            ...
            <td class="truncate px-3 py-3">
              <span
                class="block truncate text-xs font-semibold text-slate-400 transition-colors dark:text-slate-500"
                :title="b.folder_path"
              >
                {{ b.folder_path || '—' }}
              </span>
            ...
            <td class="px-3 py-3">
              <div class="flex flex-wrap gap-1">
                ...
                <span v-if="!b.tags.length" class="text-[10px] text-slate-400 dark:text-slate-500">
                  —
                </span>
              </div>
            </td>
            <td class="px-3 py-3 text-right">
              <div class="flex justify-end gap-1">
                <button
                  type="button"
                  @click="emit('edit', b)"
                  class="rounded-lg px-2.5 py-1.5 text-xs font-black text-slate-500 transition hover:bg-cyan-50 hover:text-cyan-700 dark:hover:bg-cyan-900/10 dark:hover:text-cyan-400"
                >
                  Edit
                </button>
                <button
                  ...
                  class="rounded-lg px-2.5 py-1.5 text-xs font-black text-red-500 transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
                >
                  Delete
                </button>
              </div>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:hidden">
      ...
      >
        <div
          class="flex items-start justify-between gap-3 border-b border-gray-100 bg-gradient-to-br from-slate-900 to-slate-800 p-3 text-white transition-colors dark:from-slate-950 dark:to-slate-900"
        >
          <div
          ...
          <div class="text-right">
            ...
            <a
              ...
              target="_blank"
              rel="noopener"
              class="line-clamp-1 text-[10px] text-cyan-200 hover:text-cyan-50"
            >
              {{ formatHost(b.url) }}
            </a>
          </div>
        ...
        <div class="flex flex-1 flex-col p-3">
          <a
            ...
            target="_blank"
            rel="noopener"
            class="truncate text-xs font-bold text-blue-600 transition-colors hover:underline dark:text-blue-400"
          >
            {{ b.url }}
          </a>
          <p
            v-if="b.folder_path"
            class="mt-1.5 text-[10px] font-semibold text-slate-400 dark:text-slate-500"
          >
            {{ b.folder_path }}
          </p>
          <p
          ...
          <div
            class="mt-3 flex gap-2 border-t border-gray-100 pt-3 transition-colors dark:border-slate-700/50"
          >
            <button
              type="button"
              @click="emit('edit', b)"
              class="flex-1 rounded-lg bg-gray-100 px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-gray-200 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
            >
              Edit
            </button>
            <button
              type="button"
              @click="emit('delete', b.id)"
              class="flex-1 rounded-lg bg-red-50 px-3 py-2 text-xs font-black text-red-700 transition hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30"
            >
              Delete
            </button>
          </div>
