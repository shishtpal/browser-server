<template>
  <Modal :open="open" :title="editingTodo ? 'Edit Todo' : 'New Todo'" @close="close">
    <form class="flex flex-col gap-3" @submit.prevent="onSubmit">
      <!-- Title -->
      <div>
        <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Title</label>
        <input
          v-model="form.title"
          type="text"
          required
      ...
      <!-- Description -->
      <div>
        <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Description</label>
        <textarea
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Start Date</label>
          <input
        ...
        <div>
          <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">End Date</label>
          <input
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Priority</label>
          <select
        ...
        <div v-if="editingTodo">
          <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Status</label>
          <select
            v-model="form.status"
            class="w-full rounded-lg border border-gray-300 bg-gray-50 px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:focus:ring-indigo-900/30"
          >
      ...
      <!-- Domain (Category) -->
      <div>
        <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
          <span class="flex items-center gap-1">
            <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" /></svg>
            Domain / Category
          </span>
        </label>
        <input
      ...
      <div class="grid grid-cols-[1fr_auto] gap-3 items-end">
        <div>
          <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
            <span class="flex items-center gap-1">
              <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 1 21 5 17 9" /><path d="M3 11V9a4 4 0 014-4h14" /><polyline points="7 23 3 19 7 15" /><path d="M21 13v2a4 4 0 01-4 4H3" /></svg>
              Recurrence
            </span>
          </label>
          <select
        ...
        <div>
          <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Color</label>
          <div class="flex flex-wrap gap-1">
            <button
              ...
              :class="form.color === c ? 'border-slate-900 dark:border-white scale-110' : 'border-transparent hover:scale-105'"
      ...
      <div>
        <label class="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
          <span class="flex items-center gap-1">
            <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V7h5l6.59 6.59a2 2 0 010 2.82z" /><line x1="7" y1="7" x2="7.01" y2="7" /></svg>
            Tags
          </span>
        </label>
        ...
        <div v-if="tags.length > 0" class="mt-1.5 flex flex-wrap gap-1">
          ...
          >
            {{ tag }}
            <button type="button" @click="removeTag(tag)" class="hover:text-indigo-900 dark:hover:text-indigo-200">
              <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
            </button>
          </span>
        </div>
      </div>
      ...
      <div v-if="editingTodo" class="flex items-center gap-4 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-800/50">
      ...
      <div class="flex items-center justify-between pt-2">
        <Button v-if="editingTodo" type="button" variant="danger" size="sm" @click="emit('delete')">Delete</Button>
        <div v-else></div>
        <div class="flex items-center gap-2">
          <Button type="button" variant="secondary" size="sm" @click="close">Cancel</Button>
          <Button type="submit" variant="gradient-violet" size="sm" :loading="saving">{{ editingTodo ? 'Save' : 'Create' }}</Button>
        </div>
      </div>
    </form>
  </Modal>
