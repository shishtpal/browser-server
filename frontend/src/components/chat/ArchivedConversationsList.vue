<template>
  <section class="mt-4 border-t border-slate-200 pt-3 dark:border-white/10">
    <button
      ...
      @click="$emit('toggle')"
    >
      <svg class="h-3.5 w-3.5 transition-transform" :class="{ 'rotate-90': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
      Archived
      <span v-if="items.length > 0" class="ml-auto rounded-full bg-slate-200 px-1.5 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-white/10 dark:text-slate-300">{{ items.length }}</span>
    </button>

    <div v-if="open" class="mt-1 max-h-100 space-y-1 overflow-y-auto pr-1">
      <div v-if="items.length === 0" class="px-2 py-3 text-center text-xs text-slate-400 dark:text-slate-500">
