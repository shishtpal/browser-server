<template>
  ...
  <article
    ...
    class="group relative ml-auto max-w-[82%] rounded-xl rounded-br-sm bg-indigo-600 px-3.5 py-2.5 text-white shadow-sm ring-1 ring-indigo-500/40 ring-inset dark:bg-indigo-600 dark:ring-indigo-400/30"
  >
    <!-- Image attachments -->
    ...
    <div
      class="absolute right-1 -bottom-3 hidden items-center gap-0.5 rounded-lg border border-slate-200 bg-white p-0.5 shadow-sm group-hover:flex dark:border-white/10 dark:bg-slate-900"
    >
      <button
  ...
  >
    ...
    <div
      class="absolute right-2 -bottom-3 hidden items-center gap-0.5 rounded-lg border border-slate-200 bg-white p-0.5 shadow-sm group-hover:flex dark:border-white/10 dark:bg-slate-900"
    >
      <button
        class="rounded-md p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-slate-200"
        title="Copy"
  ...
  >
    <button
      class="absolute top-0 right-0 hidden rounded-md p-1.5 text-slate-400 transition group-hover:block hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10 dark:hover:text-red-400"
      title="Delete message"
      type="button"
      @click="$emit('delete', message.id)"
    >
    ...
    <button
      class="group flex w-full items-center gap-1.5 rounded-md py-1 text-left text-[0.8em]"
      type="button"
    ...
    >
      ...
      <span class="text-slate-400 dark:text-slate-500">{{
        isRetryCall ? 'requested' : 'used'
      }}</span>
      <span class="font-mono font-medium text-slate-700 dark:text-slate-200">{{ toolLabel }}</span>
      <span class="ml-0.5 font-medium" :class="toolStatus.className"
    ...
    <div v-show="expanded" class="mt-2 space-y-2">
      <ChatQuestionForm
        v-if="isQuestionCall && message.status === 'pending' && !toolData.decision"
      ...
      >
        <p class="mb-2 text-[0.85em] text-amber-800 dark:text-amber-200">
          {{
            isRetryCall
              ...
              : 'Review the command or arguments before allowing this tool.'
          }}
        </p>
        <div class="flex flex-wrap items-center gap-2">
          <button
            ...
            type="button"
            @click="emit('tool-decision', message.tool_call_id || '', true, '')"
          >
            {{ isRetryCall ? 'Resume' : 'Allow' }}
          </button>
          <button
            ...
            type="button"
            @click="emit('tool-decision', message.tool_call_id || '', false, '')"
          >
            {{ isRetryCall ? 'Stop' : 'Reject' }}
          </button>
          <input
            ...
            :placeholder="
              isRetryCall ? 'Or add instructions before resuming…' : 'Or send feedback instead…'
            "
      ...
      <div
        v-if="toolData.decision === 'commented'"
  ...
  <Teleport v-if="previewAttachment" to="body">
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          class="absolute top-2 right-2 rounded-full bg-black/50 p-1.5 text-white hover:bg-black/70"
