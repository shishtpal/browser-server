<template>
  <Modal
    ...
    description="View, edit, or remove messages from this conversation's memory."
    fullscreen
    @close="$emit('close')"
  >
    <div class="flex h-full flex-col overflow-hidden">
      ...
      <div class="mb-3 flex shrink-0 flex-wrap items-center gap-3">
        <!-- Select all checkbox -->
        <label
          class="flex cursor-pointer items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400"
        >
          <input
            type="checkbox"
            :checked="allSelected"
            :indeterminate="selectedCount > 0 && !allSelected"
            class="h-3.5 w-3.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 dark:border-white/20 dark:bg-slate-800"
        ...
        <template v-if="selectedCount > 0">
          <button
            ...
            class="flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-100 dark:border-red-800/50 dark:bg-red-950/30 dark:text-red-300 dark:hover:bg-red-950/50"
            type="button"
            :disabled="saving"
          ...
          <div
            v-else
            class="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 dark:border-red-900/40 dark:bg-red-950/20"
          >
            <span class="text-xs text-red-700 dark:text-red-300">
              Remove {{ selectedCount }} messages?
            </span>
            <Button
              variant="danger"
              size="sm"
              ...
              >Confirm</Button
            >
            <Button variant="ghost" size="sm" @click="confirmBulkDelete = false">Cancel</Button>
          </div>
          <button
            class="text-xs text-slate-400 underline hover:text-slate-600 dark:hover:text-slate-300"
            type="button"
        ...
        <!-- Bulk remove tool calls (existing) -->
        <template v-if="toolMessageCount > 0 && selectedCount === 0">
          <button
          ...
          >
            <span class="text-xs text-red-700 dark:text-red-300">
              Remove all {{ toolMessageCount }} tool messages?
            </span>
            <Button
              variant="danger"
              size="sm"
