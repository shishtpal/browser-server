  ...
  <div ref="container" class="chat-scroll min-h-0 flex-1 overflow-y-auto bg-slate-50/40 dark:bg-slate-950" @scroll="handleScroll">
    <div class="mx-auto w-full space-y-5 px-4 py-6 lg:px-6">
      ...
      <div v-if="messages.length === 0 && !loading" class="flex min-h-[56vh] flex-col items-center justify-center text-center">
        <div class="mb-5 grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 shadow-lg shadow-indigo-500/25">
        ...
        <p class="mt-1.5 max-w-sm text-[0.85em] text-slate-500 dark:text-slate-400">Choose a model and send a message, or try one of these:</p>
      ...
      <!-- Typing indicator -->
      <div v-if="loading && messages.length > 0" class="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 shadow-sm dark:border-white/10 dark:bg-slate-900" style="width: fit-content;">
