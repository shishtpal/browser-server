  ...
  <div ref="container" class="flex-1 overflow-y-auto" @scroll="handleScroll">
    <div class="mx-auto w-full space-y-6 px-4 py-6 lg:px-8">
      ...
      <div v-if="messages.length === 0 && !loading" class="flex min-h-[50vh] flex-col items-center justify-center text-center">
        ...
        <p class="mt-2 max-w-sm text-sm text-slate-500 dark:text-slate-400">Choose a model and send a message, or try one of these:</p>
        <div class="mt-4 grid gap-2 sm:grid-cols-2">
          <button
            v-for="suggestion in suggestions"
            :key="suggestion"
            class="rounded-lg border border-slate-200 px-4 py-3 text-left text-sm transition hover:border-slate-300 hover:bg-slate-50 dark:border-white/10 dark:hover:border-white/20 dark:hover:bg-white/5"
            type="button"
