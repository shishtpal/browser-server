    ...
    <div v-if="open" class="fixed inset-0 z-50 flex lg:hidden">
      ...
      <aside class="relative z-10 flex h-full w-80 max-w-[85vw] flex-col bg-white dark:bg-slate-900">
        ...
        <div class="flex-1 space-y-1 overflow-y-auto p-3">
          ...
          <!-- Main conversations -->
          <div v-if="conversations.length === 0" class="px-2 py-4 text-center text-xs text-slate-400 dark:text-slate-500">
            No conversations
          </div>
          <div
          ...
          <div class="border-t border-slate-200 mt-2 pt-2 dark:border-white/10">
            ...
            >
              <svg class="h-3.5 w-3.5 transition-transform" :class="{ 'rotate-90': showArchived }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
              Archived
              <span class="ml-auto rounded-full bg-slate-200 px-1.5 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-white/10 dark:text-slate-300">{{ archivedConversations.length }}</span>
            </button>
          </div>
