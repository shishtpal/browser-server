    ...
    <div v-if="open" class="fixed inset-0 z-50 flex lg:hidden">
      ...
      <aside class="relative z-10 flex h-full w-80 max-w-[85vw] flex-col bg-white dark:bg-slate-900">
        <div class="flex items-center justify-between border-b border-slate-200 p-4 dark:border-white/10">
          ...
          <button class="rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-white/10" type="button"
        ...
        <div class="flex-1 space-y-1 overflow-y-auto p-3">
          <button
            ...
            :disabled="disabled" type="button" @click="$emit('new')">+ New Chat</button>
          ...
          <div v-for="conversation in conversations" :key="'m-' + conversation.id"
            ...
            <div class="relative mt-1 flex justify-end">
              <button
                ...
                aria-label="Conversation actions" type="button" @click.stop="toggleMenu(conversation.id)">
              ...
              <div v-if="openMenuId === conversation.id"
                class="absolute right-0 top-8 z-20 w-32 rounded-lg border border-slate-200 bg-white p-1 text-sm shadow-lg dark:border-white/10 dark:bg-slate-900">
                <button class="block w-full rounded px-2 py-1.5 text-left hover:bg-slate-100 dark:hover:bg-white/10"
                  type="button" @click.stop="chooseAction('rename', conversation)">Edit</button>
                <button
          ...
          <!-- Archived toggle -->
          <div class="border-t border-slate-200 mt-2 pt-2 dark:border-white/10">
            <button
              ...
              type="button" @click="$emit('toggle-archived')">
              <svg class="h-3.5 w-3.5 transition-transform" :class="{ 'rotate-90': showArchived }" fill="none"
          ...
          <div v-if="showArchived && archivedConversations.length > 0" class="space-y-1">
            <div v-for="conversation in archivedConversations" :key="'m-archived-' + conversation.id"
              class="cursor-pointer rounded-lg p-3 transition border border-slate-200 bg-slate-50/50 dark:border-white/5 dark:bg-white/5"
              @click="$emit('select', conversation.id)">
              <span class="block truncate text-sm font-semibold text-slate-400 dark:text-slate-500">{{
                conversation.title }}</span>
              <span class="block truncate text-xs text-slate-400 dark:text-slate-500">{{ conversation.model }}</span>
              <div class="mt-1 flex gap-1">
                <button
                  ...
                  title="Restore" type="button" @click.stop="$emit('restore', conversation)">
