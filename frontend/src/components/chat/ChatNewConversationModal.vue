<template>
  <Modal :open="open" title="New Conversation" description="Choose settings for the new conversation." @close="$emit('close')">
    <form class="space-y-4" @submit.prevent="handleCreate">
      <!-- Profile -->
      <div v-if="profiles.length > 0">
        <label class="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Profile</label>
        <select
      ...
      <!-- Provider -->
      <div>
        <label class="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Provider</label>
        <select
      ...
      <!-- Model -->
      <div>
        <label class="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Model</label>
        <select
          v-model="form.model"
          class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-slate-400 dark:border-white/10 dark:bg-slate-900 dark:text-white"
        >
          <option v-for="m in currentModels" :key="m.id" :value="m.id">{{ m.label || m.id }}{{ m.supports_tools ? ' 🔧' : '' }}</option>
        </select>
      </div>

      <!-- Skills -->
      <div v-if="skills.length > 0">
        <label class="mb-1.5 block text-xs font-semibold text-slate-600 dark:text-slate-300">Skills</label>
        <div class="flex flex-wrap gap-1.5">
          <button
            ...
            :class="form.skills.includes(skill.name)
