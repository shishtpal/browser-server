<template>
  <aside class="flex h-full min-h-0 flex-col border-r border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-slate-900/40">
    <div class="flex items-center justify-between gap-3 border-b border-slate-200 px-3.5 py-3 dark:border-white/10">
      <div class="min-w-0">
        <h1 class="flex items-center gap-1.5 text-[0.95rem] font-bold tracking-tight">
          <span class="grid h-5 w-5 place-items-center rounded-md bg-gradient-to-br from-indigo-500 to-violet-600 text-white">
            <svg class="h-3 w-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M21 12a9 9 0 11-4.5-7.79"/></svg>
          </span>
          AI Chat
        </h1>
      ...
      <button
        ...
        @click="$emit('new')"
      >
        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m-7-7h14"/></svg>
        New
      </button>
    </div>
    ...
    <div class="px-3.5 pt-3">
      <div class="relative">
        <svg class="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35M17 11a6 6 0 11-12 0 6 6 0 0112 0z"/></svg>
        <input
          :value="search"
    ...
    <div class="chat-scroll min-h-0 flex-1 space-y-0.5 overflow-y-auto px-2.5 py-3">
      <div v-if="conversations.length === 0" class="px-2 py-8 text-center text-[0.75rem] text-slate-400 dark:text-slate-500">
      ...
      >
        <span class="block truncate pr-6 text-[0.82rem] font-semibold leading-tight" :class="conversation.id === activeId ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-200'">{{ conversation.title }}</span>
        ...
        <div class="absolute right-1.5 top-1.5">
          <button
            ...
            :class="openMenuId === conversation.id ? 'opacity-100' : ''"
          ...
          <div v-if="openMenuId === conversation.id" class="absolute right-0 top-8 z-20 w-32 rounded-lg border border-slate-200 bg-white p-1 text-[0.8rem] shadow-lg dark:border-white/10 dark:bg-slate-900">
            <button class="block w-full rounded px-2 py-1.5 text-left font-medium hover:bg-slate-100 dark:hover:bg-white/10" type="button" @click.stop="chooseAction('rename', conversation)">Rename</button>
            <button class="block w-full rounded px-2 py-1.5 text-left font-medium text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20" type="button" @click.stop="chooseAction('archive', conversation)">Archive</button>
            <button class="block w-full rounded px-2 py-1.5 text-left font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20" type="button" @click.stop="chooseAction('delete', conversation)">Delete</button>
          </div>
        </div>
      </div>
    </div>
