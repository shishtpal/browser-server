<template>
  <div v-if="reasoning"
    ...
    <button type="button"
      ...
      <span class="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-indigo-400" v-if="streaming"></span>
      {{ streaming ? 'Thinking…' : 'Thought' }}
      <svg class="ml-auto h-3.5 w-3.5 text-slate-400 transition-transform" :class="expanded ? 'rotate-180' : ''"
        fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7" />
      </svg>
    </button>
    <div v-show="expanded"
      class="max-h-64 overflow-auto whitespace-pre-wrap break-words border-t border-slate-200/70 px-2.5 py-2 font-sans text-[0.8em] italic leading-[1.6] text-slate-500 dark:border-white/10 dark:text-slate-400">{{
        reasoning }}</div>
  </div>
</template>
