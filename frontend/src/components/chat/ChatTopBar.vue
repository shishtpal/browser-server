<template>
  <header class="flex shrink-0 flex-wrap items-center gap-2 border-b border-slate-200 bg-white/70 px-3 py-2 backdrop-blur-sm dark:border-white/10 dark:bg-slate-950/60">
    <button
      class="rounded-md border border-slate-200 p-1.5 text-slate-500 transition hover:bg-slate-50 lg:hidden dark:border-white/10 dark:hover:bg-white/5"
      type="button"
    ...
    <select
      ...
      class="h-8 rounded-md border border-slate-200 bg-white px-2 text-[0.78rem] font-medium text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/15 disabled:opacity-60 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200"
    ...
    <select
      :value="selectedModel"
      class="h-8 min-w-0 flex-1 rounded-md border border-slate-200 bg-white px-2 text-[0.78rem] font-medium text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/15 disabled:opacity-60 sm:flex-none dark:border-white/10 dark:bg-slate-900 dark:text-slate-200"
      :disabled="disabled"
    ...
    <span v-if="supportsTools" class="hidden items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[0.62rem] font-semibold uppercase tracking-wide text-amber-700 sm:flex dark:bg-amber-900/30 dark:text-amber-300">
      🔧 Tools
    </span>

    <label
      v-if="toolsEnabled"
      class="flex h-8 cursor-pointer items-center gap-1.5 rounded-md border px-2.5 text-[0.72rem] font-semibold"
    ...
    <div v-if="skills.length > 0" class="flex flex-wrap items-center gap-1">
      <button
        ...
        :key="skill.name"
        type="button"
        class="rounded-full border px-2 py-0.5 text-[0.68rem] font-medium transition"
    ...
    <span v-if="title" class="ml-auto hidden max-w-[22ch] truncate text-[0.72rem] font-medium text-slate-400 sm:block dark:text-slate-500">
      {{ title }}
    </span>

    <div class="ml-auto flex items-center gap-1 sm:ml-2">
      <button
        class="rounded-md border border-slate-200 p-1.5 text-slate-500 transition hover:bg-slate-50 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-40 dark:border-white/10 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-slate-200"
        type="button"
      ...
      <button
        class="rounded-md border p-1.5 transition"
      ...
      <!-- Tools panel toggle -->
      <button
        class="hidden rounded-md border p-1.5 transition lg:block"
