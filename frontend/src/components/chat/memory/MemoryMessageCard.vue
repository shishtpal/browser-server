<template>
  <div
    ...
    :class="[
      messageBorderClass(message, isDeleteTarget ? message.id : null),
  ...
  >
    ...
    <div class="mb-1 flex items-center gap-2">
      ...
      <span
        ...
        class="text-[10px] font-medium text-slate-600 dark:text-slate-300"
      >
        {{ toolName }}
      </span>
    ...
    <div v-else>
      <pre
        class="max-h-40 overflow-auto text-xs leading-relaxed break-words whitespace-pre-wrap text-slate-700 dark:text-slate-300"
        >{{ truncateContent(message.content) }}</pre>
    </div>
    ...
    <div
      v-if="isDeleteTarget"
      class="mt-2 flex items-center gap-2 rounded border border-red-200 bg-red-50 px-2.5 py-1.5 dark:border-red-900/40 dark:bg-red-950/20"
    >
