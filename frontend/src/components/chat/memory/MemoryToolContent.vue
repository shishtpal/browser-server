<template>
  <div class="space-y-1.5">
    <div v-if="args" class="overflow-hidden rounded border border-slate-200 dark:border-white/10">
      <header
        class="flex h-6 items-center border-b border-slate-200 bg-slate-50 px-2 dark:border-white/10 dark:bg-slate-800/60"
      >
        <span
          class="text-[9px] font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400"
        >
          Arguments
        </span>
      </header>
      <pre
        class="max-h-24 overflow-auto px-2 py-1.5 font-mono text-[11px] leading-snug break-words whitespace-pre-wrap text-slate-700 dark:text-slate-300"
        >{{ args }}</pre>
    </div>
    <div v-if="result" class="overflow-hidden rounded border border-slate-200 dark:border-white/10">
      ...
      <pre
        class="max-h-32 overflow-auto px-2 py-1.5 font-mono text-[11px] leading-snug break-words whitespace-pre-wrap text-slate-700 dark:text-slate-300"
        >{{ result }}</pre>
    </div>
