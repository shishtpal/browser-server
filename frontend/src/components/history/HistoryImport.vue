  ...
  <div class="rounded-xl border border-dashed border-gray-200 bg-white/60 p-3 shadow-sm transition-colors dark:border-white/5 dark:bg-slate-800/50">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex items-center gap-3">
        <div class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-violet-50 text-violet-600 dark:bg-violet-900/20 dark:text-violet-400">
        ...
        <div>
          ...
          <p class="text-[10px] text-slate-500 dark:text-slate-400">Upload Chrome History file (no extension)</p>
        </div>
      </div>
      <div class="flex items-center gap-2">
        <input
          ...
          accept="*/*"
          @change="onFileChange"
          class="block w-full min-w-0 max-w-40 rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs file:mr-2 file:rounded-md file:border-0 file:bg-violet-50 file:px-2 file:py-0.5 file:text-[10px] file:font-black file:text-violet-700 file:transition hover:file:bg-violet-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:file:bg-violet-900/20 dark:file:text-violet-400"
        />
        <button
          type="button"
          @click="doImport"
          ...
          :class="importing ? 'bg-slate-400' : 'bg-gradient-to-r from-violet-500 to-purple-600 hover:-translate-y-0.5 hover:shadow-md'"
        >
    ...
    <div v-if="importResult" class="mt-2 rounded-lg px-3 py-2 text-xs font-bold" :class="importResult.skipped > 0 ? 'bg-amber-50 text-amber-800 dark:bg-amber-900/20 dark:text-amber-300' : 'bg-emerald-50 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300'">
      Imported {{ importResult.imported }} history entr{{ importResult.imported !== 1 ? 'ies' : 'y' }}<span v-if="importResult.skipped > 0">, {{ importResult.skipped }} duplicate{{ importResult.skipped !== 1 ? 's' : '' }} skipped</span>
    </div>
