<template>
  <article
    ...
    @keydown.enter.prevent="$emit('open', prompt)"
    @keydown.space.prevent="$emit('open', prompt)"
  >
    <!-- hover actions -->
    <div class="absolute top-2.5 right-2.5 flex gap-1 opacity-0 transition group-hover:opacity-100">
      <button
    ...
    <div class="flex min-w-0 flex-1 flex-col" :class="dense ? 'sm:pr-0' : ''">
      ...
      <p
        ...
        class="mt-1 line-clamp-2 text-[0.76rem] text-slate-500 dark:text-slate-400"
      >
        {{ prompt.description }}
      </p>
      <p
        v-if="!dense"
        class="mt-3 line-clamp-4 rounded-lg bg-slate-50 p-2.5 font-mono text-[0.72rem] leading-5 whitespace-pre-wrap text-slate-500 dark:bg-slate-950/60 dark:text-slate-400"
      >
        {{ prompt.content }}
      </p>
      ...
      <div class="mt-auto flex flex-wrap items-center gap-1 pt-3">
        ...
        >
        <span
          v-if="(prompt.tags?.length || 0) > 4"
          class="text-[0.62rem] text-slate-400 dark:text-slate-500"
        >
