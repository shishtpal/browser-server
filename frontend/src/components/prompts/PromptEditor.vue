  ...
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 px-4 py-2.5 dark:border-white/10">
      <button
        ...
        @click="$emit('back')"
      >
        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 12H5m0 0 6 6m-6-6 6-6"/></svg>
        Back to prompts
      </button>
    ...
    <!-- Form -->
    <form class="chat-scroll flex min-h-0 flex-1 flex-col overflow-auto px-4 py-4" @submit.prevent="$emit('save')">
      <div class="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-4">
        <div>
          <label class="mb-1 block text-[0.68rem] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Title</label>
          <input
            ref="titleInputRef"
            :value="draft.title"
            class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[0.85rem] text-slate-900 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/15 dark:border-white/10 dark:bg-slate-900 dark:text-slate-100"
        ...
        <div>
          <label class="mb-1 block text-[0.68rem] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Description</label>
          <input
        ...
        <div>
          <label class="mb-1 block text-[0.68rem] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Tags</label>
          <input
            :value="tagsInput"
            class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[0.85rem] text-slate-900 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/15 dark:border-white/10 dark:bg-slate-900 dark:text-slate-100"
        ...
        <div class="flex min-h-[300px] flex-1 flex-col">
          <div class="mb-1 flex items-center justify-between">
            <label class="block text-[0.68rem] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Prompt content</label>
            <span class="text-[0.68rem] tabular-nums text-slate-400 dark:text-slate-500">{{ draft.content.length }} chars</span>
          </div>
          <textarea
            :value="draft.content"
    ...
    <!-- Footer -->
    <div class="flex items-center justify-between border-t border-slate-200 bg-white/60 px-4 py-3 dark:border-white/10 dark:bg-slate-900/50">
      <button
        v-if="draft.id"
        ...
        @click="$emit('delete')"
      >
        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0 1 16.138 21H7.862a2 2 0 0 1-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v3M4 7h16"/></svg>
        Delete
      </button>
      <span v-else></span>
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          @click="$emit('save')"
        >
          <svg v-if="isSaving" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v4a4 4 0 0 0-4 4H4z"/></svg>
