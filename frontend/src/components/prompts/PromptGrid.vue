<template>
  <div class="chat-scroll flex-1 overflow-auto bg-slate-50/40 px-4 py-4 dark:bg-slate-950/40">
    ...
    <div v-else class="grid gap-4" :class="gridClass">
      <PromptCard v-for="prompt in prompts" :key="prompt.id" :prompt="prompt" :copied="copiedId === prompt.id"
        :dense="layout === 'list'" @open="$emit('open', $event)" @use="$emit('use', $event)"
        @copy="$emit('copy', $event)" @delete="$emit('delete', $event)" />
    </div>
  </div>
</template>
