  ...
  <header
    ...
    <div class="flex min-w-0 items-center gap-3">
      ...
      <div class="min-w-0">
        <div class="flex items-center gap-2">
          <h2 class="truncate text-[0.95rem] font-bold tracking-tight text-slate-900 dark:text-white">
            {{ view === 'editor' ? (title || 'Untitled prompt') : 'Prompt Library' }}
          </h2>
          <span v-if="view === 'editor' && isDirty"
            class="rounded-full bg-amber-100 px-2 py-0.5 text-[0.6rem] font-semibold uppercase tracking-wide text-amber-700 dark:bg-amber-400/15 dark:text-amber-300">Unsaved</span>
        </div>
        <p class="truncate text-[0.72rem] text-slate-500 dark:text-slate-400">
          {{ subtitle }}
          <template v-if="view === 'grid'"> · {{ count }} prompt{{ count === 1 ? '' : 's' }}</template>
        </p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2">
      <div v-if="view === 'grid'" class="relative hidden sm:block">
        ...
        <input :value="search" type="search" placeholder="Search prompts…"
          class="w-56 rounded-lg border border-slate-200 bg-white py-1.5 pl-8 pr-3 text-[0.8rem] text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/15 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200 dark:placeholder:text-slate-500"
      ...
      <button
        ...
        type="button" aria-label="Close" @click="$emit('close')">
