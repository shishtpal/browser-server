<template>
  ...
  >
    <div class="px-4 pt-3 pb-2">
      <h3
        class="text-[0.68rem] font-semibold tracking-wider text-slate-400 uppercase dark:text-slate-500"
      >
        Tags
      </h3>
    </div>
    ...
    <nav class="chat-scroll flex-1 space-y-0.5 overflow-auto px-2 pb-4">
      ...
      <button
        ...
        :class="activeTag === null ? active : idle"
      ...
      >
        ...
        <span class="text-[0.7rem] text-slate-400 tabular-nums dark:text-slate-500">{{
          totalCount
        }}</span>
      </button>
      ...
      <button
        ...
        type="button"
        class="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[0.82rem] font-medium transition"
        :class="activeTag === UNTAGGED_FILTER ? active : idle"
        @click="$emit('select', UNTAGGED_FILTER)"
      >
        <svg
          class="h-4 w-4 shrink-0 opacity-70"
          fill="none"
      ...
      <button
        ...
        type="button"
        class="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[0.82rem] font-medium transition"
        :class="activeTag === tag.name ? active : idle"
        @click="$emit('select', tag.name)"
      >
        <span class="grid h-4 w-4 shrink-0 place-items-center text-[0.9rem] font-bold opacity-60"
          >#</span
        >
