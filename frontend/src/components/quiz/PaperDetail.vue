<template>
  ...
  >
    <div v-if="paper" class="flex h-full min-h-0 flex-col">
      ...
      <div
        class="flex-1 space-y-3 overflow-y-auto rounded-lg bg-white p-3 text-slate-900 dark:bg-slate-900 dark:text-slate-100"
      >
        <div
        ...
        >
          <p class="text-sm font-semibold">
            <span class="mr-1 font-black text-violet-600 dark:text-violet-400">{{ i + 1 }}.</span
            >{{ q.question }}
          </p>
          ...
          <ol v-else-if="q.chronology_items?.length" class="mt-2 space-y-1">
            <li
              v-for="item in ordered(q.chronology_items)"
              :key="item.index"
