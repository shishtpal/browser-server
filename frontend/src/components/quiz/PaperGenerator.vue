<template>
  <form
    class="space-y-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/60"
  ...
  >
    <label class="block">
      ...
      <input
        ...
        type="text"
        required
        placeholder="e.g. SSC Mock Test 1"
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-[11px] font-bold text-slate-600 dark:text-slate-400"
          >Sections (filters + question count)</span
        >
        <button
          type="button"
          class="rounded-md bg-violet-100 px-2 py-0.5 text-[11px] font-bold text-violet-700 transition hover:bg-violet-200 dark:bg-violet-900/30 dark:text-violet-300"
          @click="addSection"
        >
          + Add section
        </button>
      ...
      <div
        ...
        :key="i"
        class="grid grid-cols-2 gap-2 rounded-lg border border-gray-100 bg-slate-50/60 p-2 sm:grid-cols-7 dark:border-slate-700 dark:bg-slate-900/40"
      >
        <div class="flex flex-col gap-1">
          ...
          >
            ...
            <input
              ...
              placeholder="SSC"
              class="min-w-[6ch] flex-1 border-0 bg-transparent p-0 text-xs focus:outline-none dark:text-slate-200"
        ...
        <button
          type="button"
          class="rounded-lg px-2 py-1.5 text-xs font-bold text-rose-500 transition hover:bg-rose-50 disabled:opacity-40 dark:hover:bg-rose-900/20"
          :disabled="sections.length <= 1"
          @click="sections.splice(i, 1)"
        >
          Remove
        </button>
      </div>
    ...
    <p class="text-[11px] text-slate-500 dark:text-slate-400">
      Total requested: <span class="font-black">{{ totalCount }}</span> questions
    </p>
    ...
    <div class="flex justify-end">
      <button
        ...
        class="rounded-lg bg-violet-600 px-4 py-1.5 text-xs font-bold text-white transition hover:bg-violet-700 disabled:opacity-50"
      >
