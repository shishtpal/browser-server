    ...
    <div v-else class="space-y-2">
      ...
      >
        <button type="button" class="flex-1 text-left" @click="$emit('open', paper.id)">
          ...
          <p class="text-[11px] text-slate-400">{{ paper.question_count }} questions · {{ paper.sections.length }} section(s) · {{ formatDate(paper.created_at) }}</p>
        </button>
        <button type="button" class="rounded-md px-2 py-1 text-[11px] font-semibold text-rose-600 transition hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-900/30" @click="$emit('delete', paper.id)">Delete</button>
      </div>
    </div>
  </div>
</template>
