<template>
  <form class="space-y-5 sm:space-y-6" @submit.prevent="submit">
      ...
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="min-w-0">
          ...
          <span class="block text-xs text-slate-500 dark:text-slate-400">
            Tap the letter to mark
            {{ form.type === 'single_choice' ? 'exactly one' : 'one or more' }} correct
          </span>
        </div>
        <Button
          variant="secondary"
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="min-w-0">
          ...
          <span class="block text-xs text-slate-500 dark:text-slate-400"
            >Set the correct order number for each item</span
          >
        </div>
        <Button
          variant="secondary"
          ...
          :disabled="form.chronologyItems.length >= 20"
        >
          + Add item
        </Button>
      </div>
      ...
      <TransitionGroup tag="div" name="row" class="space-y-2">
        <div
          v-for="(item, i) in form.chronologyItems"
          :key="i"
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div
          class="flex flex-wrap items-center gap-1.5 rounded-lg border border-gray-300 bg-gray-50 px-3 py-2 shadow-sm focus-within:border-indigo-400 focus-within:ring-4 focus-within:ring-indigo-100 dark:border-slate-600 dark:bg-slate-800 dark:focus-within:ring-indigo-900/30"
        >
          <span
    ...
    <FormField label="Image (optional)">
      <div class="space-y-3">
        <input
          ...
          type="file"
          accept="image/*"
          class="w-full text-sm font-semibold text-slate-500 transition file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-100 file:px-4 file:py-2 file:text-sm file:font-bold file:text-indigo-700 hover:file:bg-indigo-200 dark:file:bg-indigo-900/30 dark:file:text-indigo-300 dark:hover:file:bg-indigo-900/50"
          @change="onFile"
        />
    ...
    >
      ...
      >
        <path
          fill-rule="evenodd"
          d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-4a1 1 0 112 0 1 1 0 01-2 0zm1-9a.75.75 0 00-.75.75v4.5a.75.75 0 001.5 0v-4.5A.75.75 0 0010 5z"
          clip-rule="evenodd"
        />
      </svg>
      {{ formError }}
