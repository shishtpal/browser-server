<template>
  <div>
    <div class="mb-3 flex flex-wrap items-center gap-1.5">
      ...
      />
      <select :value="filterType" class="rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" @change="$emit('update:filterType', ($event.target as HTMLSelectElement).value); $emit('applyFilters')">
      ...
      <select :value="filterDifficulty" class="rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" @change="$emit('update:filterDifficulty', ($event.target as HTMLSelectElement).value); $emit('applyFilters')">
      ...
      <div class="relative">
        ...
        <div v-if="tagPickerOpen" class="absolute z-10 mt-1 w-56 rounded-lg border border-gray-300 bg-white p-2 shadow-lg dark:border-slate-600 dark:bg-slate-800">
          <div class="mb-2 flex flex-wrap gap-1">
            ...
            >
              {{ tag }}
              <button type="button" class="text-violet-600 hover:text-violet-800 dark:text-violet-300" @click.stop="toggleTag(tag)">×</button>
            </span>
          </div>
          <div class="mb-2 flex gap-1">
            ...
            />
            <button type="button" class="rounded bg-slate-900 px-2 py-1 text-xs font-bold text-white dark:bg-white dark:text-slate-900" @click="addDraftTag">+</button>
          </div>
          ...
          <button type="button" class="mt-2 w-full rounded bg-slate-900 px-2 py-1 text-xs font-bold text-white dark:bg-white dark:text-slate-900" @click="applyAndClose">Apply</button>
        </div>
      </div>
      <select v-if="vocabulary?.subjects?.length" :value="filterSubject" class="rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" @change="$emit('update:filterSubject', ($event.target as HTMLSelectElement).value); $emit('applyFilters')">
        <option value="">All subjects</option>
        <option v-for="subject in vocabulary.subjects" :key="subject" :value="subject">{{ subject }}</option>
      </select>
      <button type="button" class="rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-slate-700 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200" @click="$emit('applyFilters')">Apply</button>
    </div>
