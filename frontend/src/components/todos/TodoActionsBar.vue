<template>
  <!-- Filters Toolbar -->
  <div class="flex w-full flex-wrap items-center gap-2.5 rounded-2xl border border-slate-200/80 bg-gradient-to-r from-slate-50/80 to-white/60 px-3 py-2.5 shadow-sm backdrop-blur-sm dark:border-slate-700/60 dark:from-slate-800/60 dark:to-slate-800/30">
    ...
    <nav class="flex items-center gap-0.5 rounded-xl bg-slate-100/80 p-1 dark:bg-slate-700/50">
      <FilterPill v-for="f in filters" :key="f.value" :active="activeFilter === f.value"
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <div class="group relative">
        <svg
          class="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-indigo-500"
        ...
        <select v-model="selectedPriority"
          class="cursor-pointer appearance-none rounded-xl border border-slate-200/80 bg-white/80 py-1.5 pl-8 pr-7 text-xs font-semibold text-slate-600 shadow-sm transition-all hover:border-indigo-300 hover:shadow focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600/60 dark:bg-slate-700/60 dark:text-slate-300 dark:hover:border-indigo-500/50 dark:focus:border-indigo-500">
          <option :value="null">Priority</option>
          <option v-for="p in allPriorityOptions" :key="p.value" :value="p.value">
            {{ p.label }}
          </option>
        </select>
        <svg class="pointer-events-none absolute right-2 top-1/2 h-3 w-3 -translate-y-1/2 text-slate-400" fill="none"
    ...
    <div v-if="view === 'list'" class="ml-auto">
      <TodoSortBar :sort-field="sortField" :sort-dir="sortDir" @update:sort-field="$emit('update:sortField', $event)"
        @toggle-dir="$emit('toggle-dir')" />
    </div>
  </div>

  <!-- Active Filter Chips -->
  <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="-translate-y-1 opacity-0"
    enter-to-class="translate-y-0 opacity-100" leave-active-class="transition-all duration-150 ease-in"
    leave-from-class="translate-y-0 opacity-100" leave-to-class="-translate-y-1 opacity-0">
    <div v-if="selectedPriority || dueDateFilter || selectedTag" class="flex flex-wrap items-center gap-2">
      <span class="text-[11px] font-medium tracking-wide text-slate-400 uppercase dark:text-slate-500">
        Filters
      </span>

      <TransitionGroup enter-active-class="transition-all duration-200 ease-out" enter-from-class="scale-90 opacity-0"
        ...
        <FilterPill v-if="selectedPriority" key="priority" :active="true" variant="tag"
          ...
          <span class="flex items-center gap-1">
            ...
            {{ selectedPriority }}
            <svg class="h-3 w-3 opacity-60 transition-opacity hover:opacity-100" fill="none" stroke="currentColor"
        ...
        <FilterPill v-if="dueDateFilter" key="dueDate" :active="true" variant="tag" @click="dueDateFilter = null">
          <span class="flex items-center gap-1">
            <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
              <rect width="18" height="18" x="3" y="4" rx="2" />
        ...
        <FilterPill v-if="selectedTag" key="tag" :active="true" variant="tag" @click="selectedTag = null">
          <span class="flex items-center gap-1">
            <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
              <path d="M12 2H2v10l9.29 9.29c.94.94 2.48.94 3.42 0l6.58-6.58c.94-.94.94-2.48 0-3.42L12 2Z" />
