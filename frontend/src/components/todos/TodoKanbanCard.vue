<template>
  <div :class="['relative group rounded-lg border p-2.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md', todo.status === 'in_progress' ? 'border-blue-300 bg-blue-50/40 dark:border-blue-500/40 dark:bg-blue-900/10 hover:border-blue-400' : 'border-gray-200 bg-white hover:border-indigo-200 dark:border-slate-700/80 dark:bg-slate-800/90 dark:hover:border-indigo-500/30']">
    ...
    <div class="flex items-start gap-2 pr-6">
      ...
      >
        <svg v-if="todo.status === 'completed'" class="h-2.5 w-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
        </svg>
        <svg v-else-if="todo.status === 'in_progress'" class="h-2.5 w-2.5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="animation-duration: 2s">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 3a9 9 0 019 9" />
        </svg>
      </button>
      <div class="min-w-0 flex-1">
        <span :class="['block truncate text-xs font-black', todo.status === 'completed' ? 'text-slate-400 line-through dark:text-slate-500' : todo.status === 'in_progress' ? 'text-blue-700 dark:text-blue-300' : 'text-slate-900 dark:text-white']">
          {{ todo.title }}
        </span>
