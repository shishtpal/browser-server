<template>
  <article class="rounded-xl border border-gray-200/80 bg-white/90 p-3 shadow-sm transition-colors dark:border-slate-700/80 dark:bg-slate-800/90">
    ...
    <div class="mt-2 flex flex-wrap items-center gap-2">
      <span class="rounded-md bg-gray-100 px-2 py-0.5 font-mono text-xs text-slate-600 transition-colors dark:bg-slate-700 dark:text-slate-300">{{ revealed ? entry.password : maskPassword(entry.password) }}</span>
      <button type="button" @click="revealed = !revealed" class="text-xs font-black text-emerald-700 transition-colors dark:text-emerald-400">{{ revealed ? 'Hide' : 'Reveal' }}</button>
    </div>
