<template>
  <tr class="transition hover:bg-emerald-50/60 dark:hover:bg-emerald-900/20">
    ...
    <td class="px-3 py-3 text-sm font-mono text-slate-600 transition-colors dark:text-slate-400">
      <span class="rounded-md bg-gray-100 px-2 py-1 transition-colors dark:bg-slate-700">{{ revealed ? entry.password : maskPassword(entry.password) }}</span>
      <button type="button" @click="revealed = !revealed" class="ml-1 inline-grid h-7 w-7 place-items-center rounded-lg text-slate-400 transition hover:bg-white hover:text-emerald-600 dark:hover:bg-slate-700 dark:hover:text-emerald-400">
