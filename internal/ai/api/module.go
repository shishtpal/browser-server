...
func (m *Module) Register(r *mux.Router) {
	...
	r.HandleFunc("/ai/videos/config", m.VideoConfig).Methods("GET")
	r.HandleFunc("/ai/videos", m.ListVideos).Methods("GET")
	r.HandleFunc("/ai/videos", m.GenerateVideo).Methods("POST")
	r.HandleFunc("/ai/videos/{id}", m.DeleteVideo).Methods("DELETE")
	r.HandleFunc("/ai/videos/{id}/file", m.GetVideoFile).Methods("GET")
