...
type route struct { server, original string; session *sdk.ClientSession; timeout time.Duration }
type managedServer struct { name string; session *sdk.ClientSession; status ServerStatus }
...
func NewManager(ctx context.Context, cfg *Config) (*Manager, error) {
	if cfg == nil { return nil, errors.New("nil MCP configuration") }
	m := &Manager{configured: cfg.Configured, routes: make(map[string]route)}
	names := make([]string, 0, len(cfg.MCPServers))
	for name := range cfg.MCPServers { names = append(names, name) }
	sort.Strings(names)
	results := make(chan *managedServer, len(names))
	var wg sync.WaitGroup
	for _, name := range names {
		sc := cfg.MCPServers[name]
		if !sc.enabled() { results <- &managedServer{name: name, status: ServerStatus{Name: name, Status: "disabled"}}; continue }
		wg.Add(1)
		go func() { defer wg.Done(); results <- connectServer(ctx, name, sc) }()
	}
	wg.Wait(); close(results)
	for s := range results { m.servers = append(m.servers, s) }
	sort.Slice(m.servers, func(i, j int) bool { return m.servers[i].name < m.servers[j].name })
	for _, s := range m.servers {
		if s.session == nil { continue }
		for _, t := range s.status.discovered {
			if len(m.tools) >= maxTools { s.status.Warnings = append(s.status.Warnings, "tool limit reached"); break }
...
type pendingTool struct { original, description string; schema json.RawMessage; timeout time.Duration }
...
func connectServer(parent context.Context, name string, sc ServerConfig) *managedServer {
	...
	if err != nil { ms.status.Error = "transport setup failed"; return ms }
	...
	if err != nil { ms.status.Error = sanitizedConnectionError(err); return ms }
	ms.session = session
	allowed := make(map[string]bool, len(sc.AllowedTools))
	for _, n := range sc.AllowedTools { allowed[n] = true }
	seen := make(map[string]bool)
	for tool, listErr := range session.Tools(ctx, nil) {
		if listErr != nil { ms.status.Status = "unavailable"; ms.status.Error = "tool discovery failed"; _ = session.Close(); ms.session = nil; return ms }
		if tool == nil || tool.Name == "" || len(allowed) != 0 && !allowed[tool.Name] { continue }
		if seen[tool.Name] { ms.status.Warnings = append(ms.status.Warnings, fmt.Sprintf("tool %q was discovered more than once", safeLabel(tool.Name))); continue }
		seen[tool.Name] = true
		schema, schemaErr := validSchema(tool.InputSchema)
		if schemaErr != nil { ms.status.Warnings = append(ms.status.Warnings, fmt.Sprintf("tool %q has an invalid schema", safeLabel(tool.Name))); continue }
		desc := tool.Description
		if len(desc) > 4096 { desc = desc[:4096] }
	...
	for n := range allowed { if !seen[n] { ms.status.Warnings = append(ms.status.Warnings, fmt.Sprintf("allowed tool %q was not discovered", safeLabel(n))) } }
...
func makeTransport(serverName string, sc ServerConfig) (sdk.Transport, error) {
	...
	if len(sc.Headers) != 0 { rt = headerTransport{base: rt, headers: sc.Headers} }
	...
	client := &http.Client{
		...
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			...
			if req.URL.Scheme != origin.Scheme || !strings.EqualFold(req.URL.Host, origin.Host) { return errors.New("cross-origin redirect refused") }
			return nil
		},
	}
...
func (w *boundedStderrWriter) Write(p []byte) (int, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if w.remaining <= 0 { return len(p), nil }
	message := string(p)
	...
	if len(message) > w.remaining { message = message[:w.remaining] }
	w.remaining -= len(message)
	message = strings.TrimSpace(message)
	if message != "" { log.Printf("MCP stdio (%s): %s", safeLabel(w.server), message) }
	return len(p), nil
}
...
func mergedEnvironment(overrides map[string]string) []string {
	...
	for _, entry := range os.Environ() {
		...
		if separator == 0 { if next := strings.IndexByte(entry[1:], '='); next >= 0 { separator = next + 1 } }
	...
	for canonical := range keys { ordered = append(ordered, canonical) }
	sort.Strings(ordered)
	out := make([]string, 0, len(ordered))
	for _, canonical := range ordered { out = append(out, keys[canonical]+"="+values[canonical]) }
	return out
}

func environmentKey(key string) string {
	if runtime.GOOS == "windows" { return strings.ToUpper(key) }
	return key
}

type headerTransport struct { base http.RoundTripper; headers map[string]string }
func (h headerTransport) RoundTrip(r *http.Request) (*http.Response, error) {
	clone := r.Clone(r.Context()); clone.Header = r.Header.Clone()
	for k, v := range h.headers { clone.Header.Set(k, v) }
	return h.base.RoundTrip(clone)
}

func validSchema(value any) (json.RawMessage, error) {
	b, err := json.Marshal(value); if err != nil || len(b) > 64*1024 { return nil, errors.New("invalid schema") }
	var object map[string]any
	if json.Unmarshal(b, &object) != nil || object == nil { return nil, errors.New("schema must be an object") }
	if typ, ok := object["type"]; ok && typ != "object" { return nil, errors.New("schema type must be object") }
	return b, nil
}

func safeLabel(s string) string { if len(s) > 100 { return s[:100] }; return s }
func sanitizedConnectionError(err error) string {
	if errors.Is(err, context.DeadlineExceeded) { return "connection timed out" }
	if errors.Is(err, context.Canceled) { return "connection canceled" }
...
func publicName(server, tool string) string {
	...
	n := normalizeName(raw)
	if n != raw || len(n) > 64 { return hashedName(n, raw) }
	return n
}
func publicNameHashed(server, tool string) string { raw := "mcp_"+server+"_"+tool; return hashedName(normalizeName(raw), raw) }
func normalizeName(s string) string {
	var b strings.Builder
	for _, r := range s { if r >= 'a' && r <= 'z' || r >= 'A' && r <= 'Z' || r >= '0' && r <= '9' || r == '_' || r == '-' { b.WriteRune(r) } else { b.WriteByte('_') } }
	n := strings.Trim(b.String(), "_-"); if n == "" { return "mcp_tool" }; return n
}
func hashedName(base, raw string) string {
	sum := sha256.Sum256([]byte(raw)); suffix := "_" + hex.EncodeToString(sum[:5])
	if len(base) > 64-len(suffix) { base = base[:64-len(suffix)] }
...
func (m *Manager) Tools() []DiscoveredTool { m.mu.RLock(); defer m.mu.RUnlock(); return append([]DiscoveredTool(nil), m.tools...) }
...
func (m *Manager) Execute(ctx context.Context, publicName string, raw json.RawMessage) (any, error) {
	m.mu.RLock(); r, ok := m.routes[publicName]; m.mu.RUnlock()
	if !ok { return nil, fmt.Errorf("unknown MCP tool %q", publicName) }
	var args map[string]any
	if len(raw) == 0 { args = map[string]any{} } else if err := json.Unmarshal(raw, &args); err != nil || args == nil { return nil, errors.New("MCP tool arguments must be a JSON object") }
	callCtx, cancel := context.WithTimeout(ctx, r.timeout); defer cancel()
	result, err := r.session.CallTool(callCtx, &sdk.CallToolParams{Name: r.original, Arguments: args})
	if err != nil { return nil, fmt.Errorf("MCP server %q tool %q call failed: %w", r.server, r.original, sanitizeCallError(err)) }
	return normalizeResult(result)
}

func sanitizeCallError(err error) error {
	if errors.Is(err, context.DeadlineExceeded) { return errors.New("timed out") }
	if errors.Is(err, context.Canceled) { return errors.New("canceled") }
...
func (m *Manager) Close() error {
	if m == nil { return nil }
	m.closeOnce.Do(func() {
		for _, s := range m.servers { if s.session != nil && s.session.Close() != nil { m.closeErr = errors.New("one or more MCP sessions failed to close") } }
	})
	return m.closeErr
}
