...
func TestExecuteWithInMemoryTransport(t *testing.T) {
	...
	if !strings.Contains(string(b), "hello") { t.Fatalf("result=%s", b) }
...
func TestProviderSafeNamesAreStableAndDistinct(t *testing.T) {
	...
	if a == b || len(a) > 64 || len(b) > 64 { t.Fatalf("invalid names %q %q", a, b) }
	if a != publicName("ServerName", strings.Repeat("tool!", 30)) { t.Fatal("name is unstable") }
}
...
func TestNormalizeResultErrorAndBinaryMetadata(t *testing.T) {
	...
	if err == nil || err.Error() != "tool failed" { t.Fatalf("error=%v", err) }
	...
	if err != nil { t.Fatal(err) }
	b, _ := json.Marshal(got)
	if strings.Contains(string(b), "secret") || !strings.Contains(string(b), "image/png") { t.Fatalf("result=%s", b) }
}
