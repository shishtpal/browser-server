...
func normalizeResult(result *sdk.CallToolResult) (any, error) {
	if result == nil {
		return nil, errors.New("MCP tool returned no result")
	}
	envelope := map[string]any{"content": normalizeContent(result.Content)}
	if result.StructuredContent != nil {
		envelope["structured_content"] = result.StructuredContent
	}
	...
	if len(encoded) > maxResultBytes {
		return nil, errors.New("MCP tool result exceeds 512 KiB")
	}
	if result.IsError {
		message := "MCP tool reported an error"
		for _, c := range result.Content {
			if text, ok := c.(*sdk.TextContent); ok && text.Text != "" {
				message = text.Text
				break
			}
		}
...
func normalizeContent(content []sdk.Content) []any {
	...
	for _, item := range content {
		...
		if typ, _ := value["type"].(string); typ == "image" || typ == "audio" {
			delete(value, "data")
		}
