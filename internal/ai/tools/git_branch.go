...
func registerGitBranch(r *Registry, paths config.PathsConfig) {
	r.add(Tool{
		Name:        "git_branch",
		Category:    "Git Operations",
		Description: "Manage git branches: list, create, delete, or rename",
		Schema:      json.RawMessage(gitBranchSchema),
		Execute:     gitBranch(paths),
	})
}

func gitBranch(paths config.PathsConfig) func(ctx context.Context, raw json.RawMessage) (any, error) {
	return func(ctx context.Context, raw json.RawMessage) (any, error) {
		...
		case "list", "":
			...
			output, err := runGit(ctx, a.WorkingDir, paths, args...)
			if err != nil {
				return nil, err
			}
		...
		case "delete":
			...
			if _, err := runGit(ctx, a.WorkingDir, paths, "branch", flag, a.Name); err != nil {
				return nil, err
			}
		...
		case "rename":
			...
			if _, err := runGit(ctx, a.WorkingDir, paths, "branch", "-m", a.Name, a.NewName); err != nil {
				return nil, err
			}
