...
func registerGitCommit(r *Registry, paths config.PathsConfig) {
	r.add(Tool{
		Name:        "git_commit",
		Category:    "Git Operations",
		Description: "Create a git commit, optionally staging files first",
		Schema:      json.RawMessage(gitCommitSchema),
		Execute:     gitCommit(paths),
	})
}

func gitCommit(paths config.PathsConfig) func(ctx context.Context, raw json.RawMessage) (any, error) {
	return func(ctx context.Context, raw json.RawMessage) (any, error) {
		...
		if a.All {
			if _, err := runGit(ctx, a.WorkingDir, paths, "add", "-A"); err != nil {
		...
		// Read back HEAD info
		info, err := runGit(ctx, a.WorkingDir, paths, "log", "-1", "--pretty=format:%H%x00%s%x00%an%x00%aI")
		if err != nil {
